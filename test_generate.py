...
def main():
    ...
    parser.add_argument("--output", default="test_submission.csv", help="Output CSV path")
    ...
    for row in tqdm(df.itertuples(), desc="Generating"):

        text = row.err_sentence
        
        id_sentences.append(row.id)
    ...
    out_df = pd.DataFrame({"id": id_sentences, "err_sentence": err_sentences, "cor_sentence": cor_sentences})
